  ...
  useEffect(() => {
    const loadEmployeeEvaluation = async () => {
      if (employee) {
        ...
        try {
          const { evaluation_id } = location.state || {};

          const res = await axiosInstance.get(`performance/evaluations/${evaluation_id}/`);
          ...
          setScores(metricFields.map(field => evalData.metrics[field] ?? ""));
